...
3. **一句一新**：一個句子最多只放**一種新東西**——本課新單字（第一次出現），或新文法點（第一次出現），**不要同一句兩者都有**。
...
5. **文法分配**：每篇最多 **2 個「新」文法點**（該點第一次出現），其餘標記都是複習；每個文法點盡量在**兩篇以上**出現。
   簡單的先教（篇一二），偏難的放後面的篇。
6. **文體用です・ます體**（N5 最熟）。引用句或該文法要求原形時可放在「」內用普通形。
   注意：驗證器比對文法字樣時，`～なければならない` 的詞幹是「なければならな」，寫成「なければなりません」會比對失敗——
   這種文法放進「」內用普通形（例：`先生に「作らなければならない」と言われました。`）。
7. **連貫、不突兀**：固定人物（我、老師、朋友…）、每篇一個場景；用 N5 的熟字句子當上下文，讓生字有前後文可以猜。
   **不要為了湊字寫突兀的句子**。第一次寫不進去的字，放到最近的場景，寧可改場景，也別硬造句子。
8. 每個目標單字至少出現 1 次；選定的文法點每個至少用 1 次。
