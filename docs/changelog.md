...
- **自我檢核修正**（做完五階段後再挑一輪）：單字卡的錄音鈕沒接到事件（sheet 只抓 `data-act`/`data-set`）；四選一答完切分頁再回來評分列會誤現；空題庫時四選一按鈕沒清；驗證器把「停用」條目當錯誤，跟「舊課不用改」的規則矛盾（改成 ⚠）；同時開兩個分頁時 store 快取不會更新、後寫的整包蓋掉先寫的（加 `storage` 事件讓快取作廢）；壞掉的紀錄（`box` 不是數字、`due` 不是日期）載入時清理；事件流上限 20,000 → 10,000（每次評分都整包序列化，手機會卡）；文法查詢頁對「A／B」型標題對不到進度；動詞工具例外表重複；桌機版分頁列是 sticky 卻留了 fixed 用的底部空白。

- **翻譯補齊與規範化**：`boarding-house` 篇一第一段只翻了 4 句中的第 1 句（字數比 0.25），其餘段落有翻譯腔（「記在了…上」「在外賃屋而居」「說「…」笑了笑」）。10 段全部重寫：一句對一句、台灣口語、文法點語氣留在中文裡（てしまう＝不小心…了、ておく＝先…起來、ようになる＝變得會…、のに＝明明…卻、たびに＝每次…都）。`validate-lessons.py` 逐段檢查翻譯：字數比 < 0.45 或漏超過一句 → 報錯；句數不同、比例 > 1.4、翻譯腔字樣 → 提醒（用舊資料驗證：擋下篇一段一、提醒兩處翻譯腔）。`SKILL.md` 步驟 4 加翻譯規範（含正誤對照）、自檢腳本逐段印句數與字數比並標「← 漏翻？」、步驟 11 加逐段檢核；`lesson-authoring.md` 加翻譯標準。
...
- **第 5 階段**：`boarding-house`、`business-trip` 補複習字（各 6）、會話（各 8 句，每人不同聲音）、文章文法、JLPT／例句假名；依自然度審查修原課幾處（「駅から細い道を通って」、篇二語序、「また、この町に」、`business-trip` 篇一「教室の前で」、5 句例句）。引擎：複習字來自舊課（中文 id）時不去抓不存在的 JSON。
