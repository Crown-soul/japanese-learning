...
- **模考產生器：規劃文件**（`docs/mock-exam-plan.md`）：昨天只在 `lesson-authoring.md` 記了五種 N4 文字語彙題型與一句「想做模考產生器」，沒有設計。這次寫成完整規劃——確認**網頁是純靜態、執行時不可能呼叫 AI**，題目只有「程式即時算」與「事先寫好的題庫」兩種來源；五種題型裡只有漢字読み與文脈規定能從現有 `vocab.json` 欄位當場算出來，其餘三種要事先寫題庫。干擾項規則跑過原型實測：只用規則變形品質差（122 個含漢字單字只有 68 個湊得到 3 個干擾項，且造出「しょっくりょうひん」這種一看就不是日文的選項）；只用別的單字真實讀音覆蓋率高但選項不相干；**採用混合做法**（送り仮名一致的真實讀音優先、不足才小幅變形），102 個湊得到、品質可用，剩 20 個跳過。**規劃寫完後又自我檢查一輪，抓到一個會出錯題的設計錯誤**：第一版寫「題幹用 `ex` 例句」，但 140 筆單字裡有 52 筆（37%）在自己的例句裡是活用形（`倒れる` 的例句是「倒れてしまいました」），辭書形的 `reading` 跟句子裡的形態對不起來；動詞與い形容詞幾乎全中。改成階段一只出「`dict` 原樣出現在 `ex` 裡」的安全題，實測可出題數：漢字読み 62 題、文脈規定 74 題，去重後 80 個不同單字。另補上「同一份考卷內一個單字只出一題」（兩種題型重疊 56 個字、共用同一句例句，會互相洩答案）。決定做成獨立頁 `lessons/mock-exam.html`（不套引擎，避免動 `lesson-engine.js` 觸發全課回歸測試），題庫不放 `data/lessons/`（會被課程 schema 驗到報錯），skill 留到階段三才開。**狀態：規劃完成，尚未動工。**
