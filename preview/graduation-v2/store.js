/* ============================================================
   【試用版專用】複製自 assets/store.js，只改三件事：
     1. localStorage key 改成 jl.preview.*，跟正式進度完全分開（試用版練的不會進正式紀錄）
     2. 新增 stories（每篇讀過／跟讀過）、exams（單課小考最高分）、paraNotes（段落回報）
     3. migrateLegacy 不做事（不去碰正式課的舊資料）
   若試用版被採用，這些改動要併回 assets/store.js（key 改回正式的）。
   ------------------------------------------------------------
...
(function () {
  "use strict";
  var SETTINGS_KEY = "jl.preview.settings.v1";
  var PROGRESS_KEY = "jl.preview.progress.v1";
  ...
  function listParaNotes() { var n = progress().paraNotes; return Object.keys(n).sort().map(function (k) { return { key: k, text: n[k].text, at: n[k].at }; }); }
