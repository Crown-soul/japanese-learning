...
READING_FIXES = {
    ...
    "二日おき": "ふつかおき",
    "五分": "ごふん",     # 會被讀成 ごぶん
    "駅から通って": "えきからとおって",  # 同段有「通うために」，「通って」可能被讀成 かよって
    "道を通って": "みちをとおって",
    "に行った": "にいった",  # 會被讀成 おこなった。收窄成「に行った」，免得把「流行った」變成「流いった」
    "行きます": "いきます",
    "行きました": "いきました",
    "写しました": "うつしました",  # 「写」會被讀成 しゃ
    "写した": "うつした",
    "楽になり": "らくになり",  # 會被讀成 たのしく系。收窄成「楽になり」，免得把「音楽に」變成「音らくに」
    "粗い": "あらい",     # 罕用漢字，會被讀成 そ―
    "餌": "えさ",        # 會被讀成 じ
    # --- business-trip 課（初めての出張）預填 ---
    "連れて行って": "つれていって",  # 「行って」會被讀成 おこなって
    "連れて行く": "つれていく",    # 同上，辭書形
    "なさる方だ": "なさるかただ",   # 「方」（敬語，指人）會被讀成 ほう
    "なさる方でした": "なさるかたでした",  # 6 篇改寫版新句
    "厳しい方ですが": "きびしいかたですが",  # 「方」（指人）會被讀成 ほう
    # --- graduation 課（いよいよ卒業式）預填 ---
    "解けば解くほど": "とけばとくほど",  # 「解」會被讀成 かい
    "離してしまい": "はなしてしまい",   # 「離」會被讀成 り
    "離しました": "はなしました",
    "離して": "はなして",
    "あの日の光": "あのひのひかり",    # 「光」單獨會被讀成 こう
    "一人ずつ": "ひとりずつ",        # 「一人」會被讀成 いちにん
    "と二人で": "とふたりで",       # 「二人」會被讀成 ににん。故意收窄成「と二人で」：
    #                              boarding-house 的「帰りに二人で」音檔已產並聽過，不要因為這條重產
    "開きました": "ひらきました",     # 「開く」已在上面，但活用形不會被它蓋到，會被讀成 あきました
    "開いた": "ひらいた",
    "見直した": "みなおした",        # 「直」會被讀成 ちょく
    "南の窓": "みなみのまど",
    "撒きました": "まきました",       # 「撒」罕用漢字，會被讀成 さん
    "撒く": "まく",
    "目方": "めかた",            # 「方」會被讀成 ほう
    "夕方": "ゆうがた",           # 同上
    "日程": "にってい",           # 「日」會被讀成 ひ／にち
    "朝日": "あさひ",            # 會被讀成 ちょうにち
    "までの間に": "までのあいだに",    # 「間」單獨會被讀成 かん／ま（別寫成通用規則，
    "夏休みの間に": "なつやすみのあいだに",  #  否則會誤傷 boarding-house 的「いつの間に＝いつのまに」）
    "皆で": "みなで",            # 會被讀成 みんな
    "先に": "さきに",            # 「先」單獨會被讀成 せん
    # --- graduation 課 6 篇版改寫後預填（預防性，聽 audio-check 後再確認）---
    # 「三十分」故意收窄：business-trip 的「三十分だと…」音檔已產並聽過，不要因為這條重產
    "電車で三十分": "電車でさんじゅっぷん",  # 「分」會被讀成 ぶん（同「五分」）
    "三十分ぐらい": "さんじゅっぷんぐらい",
    "、二人で": "、ふたりで",      # 「二人」會被讀成 ににん（同「と二人で」）
}
EXACT_FIXES = {
    "日": "ひ",
}
